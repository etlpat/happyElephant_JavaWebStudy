...
public class LogInServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        for (Cookie cookie1 : cookies) {
            // 获取用户注册cookie
            if (cookie1.getValue().equals(user.getUsername())) {
                String psdName = "password" + cookie1.getName().substring("username".length(), cookie1.getName().length());
                for (Cookie cookie2 : cookies) {
                    if (cookie2.getName().equals(psdName) && cookie2.getValue().equals(user.getPassword())) {
